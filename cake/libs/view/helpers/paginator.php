	...
	function first($first = '<< first', $options = array()) {
		...
		if (is_int($first) && $params['page'] > $first) {
			...
			if($params['page'] > 1)
				...
				$out = $this->Html->tag($tag, $first, array_merge($options, array('class'=>'disabled '.$options['class'])));
		}
	...
	function last($last = 'last >>', $options = array()) {
		...
		if (is_int($last) && $params['page'] < $lower) {
			...
			if ($params['page'] < $params['pageCount'])
				$out = $this->Html->tag($tag, $this->link($last, array('page' => $params['pageCount']), $options));
			else
				$out = $this->Html->tag($tag, $last, array_merge($options, array('class'=>'disabled '.$options['class'])));
