jQuery(function($){
	...
	self = {
		createFramework: function(onlyIfNotInitialize){
			...
			// close
			$feedback.prepend($('<span class="icon-cancel close" title="' + options.closeText + '" />').click(self.close));
			
			// minimize
			if(options.minimizable){
				$feedback.prepend($('<span class="icon-minus minus" title="' + options.minimizeText + '" />').click(self.minimize));
			}
		...
		addMessage: function(data){
			...
			colors = {green: '#00B000', red: '#D92316', yellow: '#CDB907'},
			$close = $('<span class="icon-cancel close" />'),
			color = false;
