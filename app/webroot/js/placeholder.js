...
NS.Placeholder = new Class({
	...
	initialize: function(options)
	{
		...
		elements.each(function(el){
			...
			el.store('ispassword', el.get('type') == 'password');
	...
	focus: function(el, focus)
	{
		...
		if (value == '' || value == text)
		{
			if (el.retrieve('ispassword') && !(Browser.ie6 || Browser.ie7 || Browser.ie8)){
			...
			if(focus){
				...
				if((el.retrieve('ispassword') && !(Browser.ie6 || Browser.ie7 || Browser.ie8)) || !el.retrieve('ispassword'))
				//if((el.retrieve('ispassword') && !(Browser.ie6 || Browser.ie7 || Browser.ie8)) || )
					el.set('value', text);
			}
		}
